...
6. Before each leg, require the Taipei date to remain the planned date and `13:20:00 <= now < 13:25:00`. Submit a `Common` leg as `MKT` + `IOC`, then refresh and poll its status. A successor becomes eligible once one matching order record confirms either the `Common` predecessor's complete fill at a finite positive weighted price, or its `Failed`, `Inactive`, `Cancelled`, or `Rejected` status with no fill. A dependent rebuy always needs the complete fill. bt does not poll `IntradayOdd` legs.
...
- Fail-safe: an unavailable server, calendar or fetch failure, stale cache, stale or invalid snapshot, evaluation error, unsupported inventory, order-placement uncertainty, missing or ambiguous status, mismatched order fields, partial fill, timeout, or cutoff stops every remaining leg. A `Common` order rejected with no fill lets later independent legs run but blocks its dependent rebuy; see [Design: share quantum and odd lots](./share-quantum-and-odd-lots.md#tw-live-lot-and-odd-lot-orders). The daemon logs each observed trade, the pre-trade holdings, and any stop reason with the unsubmitted legs.
- Confirmation: the executor never infers a `Common` fill from a successful POST return. Before it moves on, it requires the placed order ID and one refreshed matching trade. That trade shows either `Filled` with full deal lots at a finite positive weighted price, or a no-fill rejection. A no-fill rejection still blocks a dependent rebuy.
