...
- Order discipline: every `Common` leg is `price_type: MKT`, `order_type: IOC`, and `price: 0`, matching the official Shioaji stock-order fields. `IntradayOdd` legs are limit `ROD`; see [Design: share quantum and odd lots](./share-quantum-and-odd-lots.md#tw-live-lot-and-odd-lot-orders). The executor rechecks the session and the `< 13:25` cutoff before every order and while polling it.
...
- Engine alignment: the daemon does not reinvent sizing. The engine's per-bar fill planner is an exported pure function used by both `Engine.run` and TW decision planning. TW execution additionally splits legs into lot and odd-lot orders and applies confirmed-fill cash and inventory constraints between orders.
...
- Place order: `POST /api/v1/order/place_order` accepts contract data and stock-order fields including `action`, `price`, `quantity`, `price_type`, `order_type`, `order_lot`, `order_cond`, `custom_field`, and optional `account`. The official stock-order reference permits `MKT` and `IOC`; the implemented lot request uses `price: 0`, `MKT`, `IOC`, and `Common`. The odd-lot request uses `LMT`, `ROD`, and `IntradayOdd`; see [Design: share quantum and odd lots](./share-quantum-and-odd-lots.md#market-facts).
...
- `broker/shioaji.ml` + `.mli`: REST client for server info, snapshots, aggregate positions, dated position details, balance, settlements, `Common` and `IntradayOdd` order placement, and today's trades. The base URL comes from `SHIOAJI_URL`; optional `SJ_API_KEY` and `SJ_SEC_KEY` add Bearer authentication only when both are nonempty.
- `engine/engine.ml` + `.mli`: exported pure fill planner used by the unchanged daily engine path and TW decision planning.
- `broker/live.ml` + `.mli`: TW decision, broker-derived production cash and equity, independent-calendar preparation, 18-month position-detail rollover planning, lot and odd-lot translation, and sequential confirmed-fill execution beside the unchanged US arms.
...
3. At 13:20, request a fresh snapshot and validate its session plus internally consistent finite positive OHLCV fields. Build the provisional bar, evaluate the unchanged DSL path, and compute the final and previous bar's effective TW targets. Ordinary planning is not forced: equal effective targets preserve the drifted broker inventory, while a changed target trades from that inventory.
...
6. Before each leg, require the Taipei date to remain the planned date and `13:20:00 <= now < 13:25:00`. Submit a `Common` leg as `MKT` + `IOC`, then refresh and poll its status. A successor is eligible after one matching order record confirms the `Common` predecessor's complete fill at a finite positive weighted price, or its `Failed`, `Inactive`, `Cancelled`, or `Rejected` status with no fill. A dependent rebuy always needs the complete fill. `IntradayOdd` legs are not polled.
7. Carry cash and inventory forward from confirmed `Common` fills and from the full cost of each sent odd-lot buy. A refinance rebuy requires its entire original lot count to remain funded. A capped ordinary buy stops with its remainder and every later leg unsubmitted. After 13:30, query and log today's resulting trades.

The pre-plan query for today's orders is conservative deduplication, not an exactly-once guarantee. A crash between orders or two concurrent daemons can still leave ambiguous exposure. A refinance sell and rebuy are sequential orders, not atomic. If a matured lot's sale cannot fund its complete Common-lot rebuy, dependent-leg gating stops after the sale; unlike the daily engine, which rebuys the fundable whole lots, the daemon does not partially restore that lot.

## Safety and failure

- Fail-safe: an unavailable server, calendar or fetch failure, stale cache, stale or invalid snapshot, evaluation error, unsupported inventory, order-placement uncertainty, missing or ambiguous status, mismatched order fields, partial fill, timeout, or cutoff stops every remaining leg. A `Common` order rejected with no fill lets later independent legs run but blocks its dependent rebuy; see [Design: share quantum and odd lots](./share-quantum-and-odd-lots.md#tw-live-lot-and-odd-lot-orders). The daemon logs any observed trade and resulting exposure.
- Confirmation: the executor never infers a `Common` fill from successful POST return. It requires the placed order ID and one refreshed matching trade before it moves on. The trade shows either `Filled` with full deal lots at a finite positive weighted price, or a no-fill rejection. A no-fill rejection still blocks a dependent rebuy.
